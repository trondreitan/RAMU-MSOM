...
hyper.this=list(logsd.mu=0, logsd.sd=4, 
                ...
                logit.mu=0, logit.sd=4, 
                    log.lambda.mu=0, log.lambda.sd=4)
...
init.params.flat=function(hyp)
{
  ...
  ret=list(
   log.betabin.s=rnorm(N.sp, hyper$log.betabin.s.mu, hyper$log.betabin.s.sd/4),
    log.const.lambda=rnorm(N.sp,hyper$log.lambda.mu,hyper$log.lambda.sd),   logit.const.occu=rnorm(N.sp-1,hyper$logit.mu,hyper$logit.sd),
   log.sd.f.occu=rnorm(1,hyper$logsd.mu, hyper$logsd.sd/4),
    log.sd.f.lambda=rnorm(1,hyper$logsd.mu, hyper$logsd.sd/4),     
   log.sd.sf.occu=rnorm(N.sp-1,
     hyper$logsd.mu-2, hyper$logsd.sd/4),
 ...
 for(i in 1:N.sp)
  ret$sf.lambda[,i]=rnorm(Nf, 0, 
    exp(ret$log.sd.sf.lambda[i])/4)
...
# need to send it, really...
{
  ...
  N.forms=X$N.forms
  Nf=X$Nf
  counts=Y[,(which(names(Y)=="num.with.colonies")+1):which(names(Y)=="Species_Superspecies")]
  ...
  ll=ll+sum(dlbetabin.zero(counts, nrep, 1-exp(-lambda.mat),
            betabin.s.use, occu.mat))

...
hyper.this=list(logsd.mu=0, logsd.sd=0.72, 
...
init.params.flat=function(hyp)
{
  ...
  ret=list(
   ...
   weight.shell=rnorm(1,0,1),
   weight.vol=rnorm(1,0,1),
   weight.area=rnorm(1,0,1),
   ...
   logit.reg.occu=rnorm(1*(sum(inter[1:Nf,1:(N.sp-1)]==0)>0),
      hyp$logit.mu+log(2),hyp$logit.sd/4),
   reg.occu.state=rep(0.5, sum(inter[1:Nf,1:(N.sp-1)]==0)),
...
init.params.flat.prevmodel=function(hyp,prevmodel.mu,prevmodel.sd)
{
  ...
  ret=list(
   ...
   weight.x=rnorm(1,0,1),
   weight.y=rnorm(1,0,1),
...
logprior.flat=function(par, hyp)
{
  ...
  weight.shell=par[(i+1)]; i=i+1
  weight.vol=par[(i+1)]; i=i+1
  weight.area=par[(i+1)]; i=i+1
  ...
  ret=0
  if(sum(inter[1:Nf,1:(N.sp-1)]==0)>0)
  {
    logit.reg.occu=par[(i+1)]; i=i+1
    ret=sum(dnorm(logit.reg.occu, hyp$logit.mu+log(10),hyp$logit.sd,
                  log=T))
  }
  ...
  sum(dnorm(weight.shell, 0, 1, log=T)) +
  sum(dnorm(weight.vol, 0, 1, log=T)) +
  sum(dnorm(weight.area, 0, 1, log=T)) +
  ...
  sum(dnorm(logit.gamma, hyper$logit.mu, hyper$logit.sd, log=T)) +
  ...
  sum(dnorm(log.sd.sf.lambda, mu.lsd.sf.lambda, exp(lsd.lsd.sf.lambda), log=T))
...
# need to send it, really...
{
  ...
  weight.vol=par[(i+1)]; i=i+1
  weight.area=par[(i+1)]; i=i+1
  ww=c(ilogit(weight.shell),ilogit(weight.vol), ilogit(weight.area))
  ww=ww/sum(ww)
  weight.shell=ww[1]
  weight.vol=ww[2]
  weight.area=ww[3]
  ...
  n.no.presence=sum(inter[1:Nf,1:(N.sp-1)]==0)
  if(n.no.presence>0)
  {
    logit.reg.occu=par[(i+1)]; i=i+1
    reg.occu=ilogit(logit.reg.occu)
    reg.occu.state=par[(i+1):(i+n.no.presence)]; i=i+n.no.presence
  }
  ...
  exposure=weight.shell*Y$Total+weight.vol*Y$std.weight+weight.area*Y$std.area

  nrep=exposure%*%t(rep(1,N.sp))
  #ll=ll+sum(dlbetabin.zero(counts, nrep, 1-exp(-lambda.mat), betabin.s.use, occu.mat))
  ll=ll+sum(dlnegbinom.zero(counts, nrep*lambda.mat, kappa.mat, occu.mat))

  ng=as.matrix(Y[,which(substr(names(Y),1,12)=="Unidentified")])
  tg=as.matrix(Y[,which(substr(names(Y),1,5)=="Genus")[X$unid.genus.nr]]  )
  ig=tg-ng

...
make.data=function(new.par0,hyp,y)
{
  ...
  data=list(PGF=PGF,x=NA,y=y,mon.names=mon.names,parm.names=parm.names,
     ...
     forms=hyp$forms, K=hyp$K, K1=hyp$K1, K2=hyp$K2, N.un=hyp$N.un,
     sp=hyp$sp, genera=hyp$genera, genus.nr=hyp$genus.nr,
     genusnames=hyp$genusnames, unid.genus.nr=hyp$unid.genus.nr,
     inter=inter,no.presence=hyp$no.presence,n.no.presence=hyp$n.no.presence)

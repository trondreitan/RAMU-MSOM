...
find.best.par.hpc=function(mcmc.fit,burnin=100,doshow=F)
{
  ...
  if(burnin>1.5*dim(mcmc.fit[[1]]$Monitor)[1])
    burnin=floor(dim(mcmc.fit[[1]]$Monitor)[1]/2)
...
find.best.par=function(mcmc.fit,data,burnin=100,doshow=F)
{
  ...
  if(burnin>1.5*dim(mcmc.fit$Monitor)[1])
    burnin=floor(dim(mcmc.fit$Monitor)[1]/2)
